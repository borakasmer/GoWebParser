...
func GetSqlContent(db *sql.DB) ([]string, []float64, []float64, []string, []float64, error) {
	...
	rows, err := db.Query("select Name,Price,TrPrice,ExchangeType,ExchangeValue from [dbo].[Products2]")
	if err != nil {
		log.Fatal(err)
	}
	defer rows.Close()
...
func InsertSqlContent(db *sql.DB,product *shared2.AddProduct) (int64,error){
	...
	defer stmt.Close()
	rows := stmt.QueryRow(product.Name,product.Price,product.TrPrice,1,product.ExchangeName,product.ExchangeValue)
	if rows.Err() != nil {
		return 0, err
	}
	var _id int64
